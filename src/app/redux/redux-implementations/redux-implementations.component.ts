...
@Component({
  ...
  styleUrls: [
    './redux-implementations.component.css',
    '../../shared/flex-spacer.css'
  ]
})
export class ReduxImplementationsComponent implements OnInit {
  ...
  implementations: { name: string; description: string; link: string }[] = [
    ...
    {
      name: 'NgRx',
      description: 'Library for Angular apps',
      link: 'https://github.com/ngrx/platform'
    },
    {
      name: 'VueX',
      description: 'Library for Vue.js apps',
  ...
  ngOnInit() {
    this.cols = this.breakpointObserver
      ...
      .pipe(
        map(state => {
          if (
            state.breakpoints[ReduxImplementationsComponent.WIDTH_UNDER_675PX]
          ) {
            return 1;
          } else if (
            state.breakpoints[ReduxImplementationsComponent.WIDTH_UNDER_950PX]
          ) {
            return 2;
          } else {
            return 3;
